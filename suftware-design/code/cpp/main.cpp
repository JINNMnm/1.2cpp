...
int main()
{
    ...
    int bhv, assessmentv, creditv, numofchosenv, chosenv[25], dayv, timev;
    ...
    while (inCourse >> bhv >> namev >> teacherv >> creditv >> catagoryv >> collegev >> assessmentv >> dayv >> timev)
    {
        coulist.addcourse(bhv, namev, teacherv, creditv, catagoryv, collegev, assessmentv, dayv, timev);
    }
...

    {
        ...
        out2:
            ...
            case 1:
                ...
                coulist.showcourse();
                cout << endl;
                cout << "目前已选课程编号如下,共" << target->getchosenlist().getsize() << "节：" << endl;
                target->showchosen(coulist);
            ...
            case 2:
                //退课
                if (target->getchosenlist().getsize() == 0)
                {
                    cout << "你还没有选课" << endl;
                    system("pause");
                    goto out2;
                ...
                if (target->delchosen())
                {
                    if (target->getchosenlist().getsize() > 0)
                    {
                        cout << "退课成功,以下是更新后的课程信息" << endl;
                        target->showchosen(coulist);
                    }
                    else
                    {
                        cout << "退课成功,你已经没有选课了" << endl;
                    }
                }
                else
                {
                    cout << "退课失败,你没有选这门课" << endl;
                }
                system("pause");
                stulist.write();
                goto out2;
        ...
        out3:
            ...
            case 1:
                ...
                cout << "目前已有的课程信息如下,共" << coulist.getsize() << "节：" << endl;
                coulist.showcourse();
                cout << "请按以下顺序输入课程信息：课程编号 课程名称 任课教师 学分 课程类别 开课学院 课程评价 课程日(周几) 课程时间" << endl;
                cin >> bhv >> namev >> teacherv >> creditv >> catagoryv >> collegev >> assessmentv >> dayv >> timev;
                if (coulist.findbh(bhv))
                {
                    cout << "添加失败!因为您提供的课程编号已存在" << endl;
                    system("pause");
                    goto out3;
                }
                if(!coulist.addcourse(bhv, namev, teacherv, creditv, catagoryv, collegev, assessmentv, dayv, timev)){
                    system("pause");
                    goto out3;
                }
                cout << endl
                     << "加课成功,共" << coulist.getsize() << "节,以下是加课后的课程信息汇总" << endl
                     << endl;
                coulist.showcourse();
                coulist.write();
                system("pause");
            ...
            case 2:
                ...
                cout << endl
                     << "删课成功,共" << coulist.getsize() << "节,以下是删课后的课程信息汇总" << endl
                     << endl;
                coulist.showcourse();
                system("pause");
                coulist.write();
            ...
            case 3:
                ...
                cout << endl
                     << "修改成功,共" << coulist.getsize() << "节,以下是修改后的课程信息汇总" << endl
                     << endl;
                coulist.showcourse();
                system("pause");
                coulist.write();
